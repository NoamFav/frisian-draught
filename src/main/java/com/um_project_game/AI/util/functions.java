...
public class functions {
    ...
    public void qLearningUpdate(GameState state, Vector2i action, double reward, GameState nextState, DQNModel model, DQNModel targetModel, double alpha, double gamma) {
        ...
        double maxNextQ = nextQValues.values().stream().mapToDouble(Double::doubleValue).max().orElse(0.0);
    ...
    public double computeLoss(List<Experience> miniBatch, DQNModel model, DQNModel targetModel, double gamma) {
        double totalLoss = 0.0;

        for (Experience exp : miniBatch) {
            double target = exp.isTerminal ? exp.reward : exp.reward + gamma * targetModel.predict(exp.nextState).values().stream().mapToDouble(Double::doubleValue).max().orElse(0.0);
    ...
    public double minimax(GameState state, int depth, double alpha, double beta, boolean maximizingPlayer, DQNModel dqnModel) {
        ...
        if (maximizingPlayer) {
            ...
            for (Move move : state.generateMoves()) {
                GameState newState = state.applyMove(move.getStartPosition(), move.getEndPosition()).getNextState();
    ...
    public Vector2i chooseActionEpsilonGreedy(GameState state, DQNModel model, double epsilon) {
        if (Math.random() < epsilon) {
            ...
            return moves.get(new Random().nextInt(moves.size())).getStartPosition(); // Choose a random start position as the action
        } else {
    ...
    public void trainDRLModel(DQNModel model, DQNModel targetModel, ReplayBuffer replayBuffer, int batchSize, double gamma, double alpha, int numEpochs) {
    ...
    public void storeExperience(ReplayBuffer replayBuffer, GameState state, Vector2i action, double reward, GameState nextState, boolean done) {
        replayBuffer.addExperience(new Experience(state, action, reward, nextState, done));
    }

    public void mainDRLLoop(GameState initialState, DQNModel model, DQNModel targetModel, ReplayBuffer replayBuffer, double epsilon, double gamma, double alpha) {
        GameState state = initialState;
        while (!state.isTerminal()) {
            Vector2i action = chooseActionEpsilonGreedy(state, model, epsilon);
            MoveResult result = state.applyMove(action, new Vector2i(action.x + 1, action.y + 1)); // Adjust move logic as needed
    ...
    public double evaluateStateWithDQN(GameState state, DQNModel dqnModel) {
        return dqnModel.predict(state).values().stream().mapToDouble(Double::doubleValue).max().orElse(0.0);
    }
}
