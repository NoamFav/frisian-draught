...
public class GameState {
    ...
    public double[] toInputArray() {
        ...
        System.out.println("Encoding board state: " + boardState);
    ...
    public List<Move> generateMoves() {
        ...
        for (Map.Entry<Vector2i, Pawn> entry : boardState.entrySet()) {
            ...
            // Add normal moves
            int[][] directions = pawn.isKing()
                ? new int[][]{{-1, -1}, {1, -1}, {-1, 1}, {1, 1}} // Kings move in all directions
                : isWhiteTurn
    ...
    private boolean isValidCapture(Vector2i startPosition, Vector2i capturePosition, Vector2i landingPosition) {
        // Ensure the landing position is within board boundaries
        if (landingPosition.x < 0 || landingPosition.x >= 10 || landingPosition.y < 0 || landingPosition.y >= 10) {
            return false;
        }
    
        // Ensure the capture position is within board boundaries
        if (capturePosition.x < 0 || capturePosition.x >= 10 || capturePosition.y < 0 || capturePosition.y >= 10) {
            return false;
        }
        ...
        if (capturedPawn == null || movingPawn == null || capturedPawn.isWhite() == movingPawn.isWhite()) {
            return false;
        }
    ...
    public MoveResult applyMove(Move move) {
        ...
        // Handle promotion
        if ((movingPawn.isWhite() && endPosition.y == 0) || (!movingPawn.isWhite() && endPosition.y == 9)) {
